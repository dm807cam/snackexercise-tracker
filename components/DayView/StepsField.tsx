...
export function StepsField({
  ...
  date: LocalDate;
  steps: number | null;
  onSaved: (steps: number | null) => void;
  ...
  async function save() {
    ...
    try {
      await api(`/api/metrics/${date}`, {
        method: "PUT",
        body: JSON.stringify({ steps: parsed, source: "manual" }),
      });
      onSaved(parsed);
      setEditing(false);
    } catch (error) {
      ...
      <div className="mb-3 flex justify-center">
        ...
        >
          ...
          {steps == null ? (
            ...
            <span className="tabular-nums">{steps.toLocaleString()} steps</span>
          )}
        </button>
      </div>
    );
  ...
  return (
    <div className="mb-3 flex items-center justify-center gap-2">
      ...
      <input
        ...
        className="w-32 rounded-lg px-3 py-2 text-sm tabular-nums"
        style={{ background: "var(--surface-2)", border: "1px solid var(--border)" }}
      />
      <button
        type="button"
